...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) :
        maps(vector<PartMap>(bucket_count))
    {}

    WriteAccess operator[](const K& key)
    {
        const size_t bucket = hasher(key) % maps.size();

        maps[bucket].m.lock();
        return { maps[bucket].storage[key], maps[bucket].m };
    }
    ...
    ReadAccess At(const K& key) const
    {
        ...
        maps.at(bucket).m.lock();
        if (maps.at(bucket).storage.count(key) == 0)
        {
            maps.at(bucket).m.unlock();
            throw out_of_range("no such element");
        }

        return { maps.at(bucket).storage.at(key), maps[bucket].m };
    }

    bool Has(const K& key) const
    {
        size_t bucket = hasher(key) % maps.size();
        lock_guard<mutex>(maps.at(bucket).m);
        return maps.at(bucket).storage.count(key) != 0;
    }
