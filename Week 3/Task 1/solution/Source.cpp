...
public:
    ...
    optional<string> MostPopularName(char sex) const;
...
private:
    ...
    mutable optional<string> most_popular_male_name, most_popular_female_name;
};
...
unsigned Storage::TotalIncome(size_t amount) const
{
    ...
    return accumulate(head.begin(), head.end(), 0u, [](unsigned sum, const Person& person)
        {
            return sum += person.income;
        });
}

optional<string> Storage::MostPopularName(char sex) const
{
    size_t max_ = 0;
    string result;
