...
public:
    IndexProxy() = delete;
    IndexProxy(vector<T>& cfs, size_t pos) :
        cfs_(cfs),
        pos_(pos)
    {}
...
template <typename T>
bool operator==(const IndexProxy<T>& l, const T& r)
{
    if (l.cfs_.size() <= l.pos_)
    {
        return r == 0;
    }
    else
    {
        return l.cfs_[l.pos_] == r;
    }
}
...
{
    if (index.pos_ >= index.cfs_.size())
    {
        out << 0;
    }
    else
    {
        out << index.cfs_.at(index.pos_);
    }
...
public:
    ...
    {
        return IndexProxy<T>(coeffs_, degree);
    }
